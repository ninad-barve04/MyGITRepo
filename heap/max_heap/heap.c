...
/**
 * @brief Heapify for MAX Heap after insertion. Bottom up heapification
 * 
 * @param heap  (Heap *) pointer reference to the heap
...
/**
 * @brief Heapify for MAX heap after deletion. Top down heapification
 * 
 * @param heap 
 * @param root 
 */
void heapifyDelete(Heap *heap, int root) {
    ...
    if (left < heap->next && heap->array[largest] < heap->array[left]) {
        largest = left;
    }
    
    // If right child greater than root
    if (right < heap->next && heap->array[largest] < heap->array[right]) {
        largest = right;
    }
